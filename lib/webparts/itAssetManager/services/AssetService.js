...
var AssetService = /** @class */ (function () {
    ...
    AssetService.prototype.getNextSequenceNumber = function (type, country, office) {
        return __awaiter(this, void 0, void 0, function () {
            var prefix, items, maxSeq, _i, items_1, item, parsed, next;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        prefix = AssetIdGenerator.getPrefix(type, country, office);
                        console.log("[AssetService] getNextSequenceNumber \u2014 prefix: \"".concat(prefix, "\""));
                    ...
                    case 1:
                        items = _a.sent();
                        if (!items.length) {
                            console.log("[AssetService] No existing assets found for prefix \"".concat(prefix, "\" \u2014 starting at 1"));
                            return [2 /*return*/, 1];
                        }
                        console.log("[AssetService] Found ".concat(items.length, " existing asset(s) for prefix \"").concat(prefix, "\""));
                        maxSeq = 0;
                        for (_i = 0, items_1 = items; _i < items_1.length; _i++) {
                            ...
                            if (parsed && parsed.sequence > maxSeq) {
                                console.log("[AssetService] Fallback parse: Title=\"".concat(item.Title, "\" \u2192 sequence=").concat(parsed.sequence));
                                maxSeq = parsed.sequence;
                            }
                        }
                        next = maxSeq + 1;
                        console.log("[AssetService] Max SequenceNumber found: ".concat(maxSeq, " \u2014 next sequence: ").concat(next));
