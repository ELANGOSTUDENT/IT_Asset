...
import { AttachRegular, TagRegular, WrenchRegular, MoneyRegular, DocumentRegular, ShieldCheckmarkRegular, ClockRegular, ArchiveRegular, } from '@fluentui/react-icons';
import { ASSET_TYPE_LABELS, NEW_ASSET_TYPES, COUNTRY_OPTIONS, CITY_OPTIONS, OFFICE_OPTIONS_BY_CITY, CITY_CODE_FROM_OFFICE, MAKE_OPTIONS, MODEL_TYPE_OPTIONS, WARRANTY_TYPE_OPTIONS, MAINTENANCE_TYPE_OPTIONS, EOS_REASON_OPTIONS, } from '../models/IAsset';
...
var RepairDialog = function (_a) {
    ...
    return (React.createElement(Dialog, { hidden: !open, onDismiss: onClose, dialogContentProps: {
        ...
        React.createElement("div", { className: styles.dialogGrid },
            React.createElement(Dropdown, { label: "Type", selectedKey: draft.MaintenanceType || '', options: __spreadArray([
                    { key: '', text: 'Select type…' }
                ], MAINTENANCE_TYPE_OPTIONS.map(function (t) { return ({ key: t, text: t }); }), true), onChange: function (_e, opt) { return set('MaintenanceType', (opt === null || opt === void 0 ? void 0 : opt.key) || ''); } }),
            React.createElement(DateField, { label: "Date *", value: draft.RepairDate, onChange: function (v) { return set('RepairDate', v); } }),
            ...
            React.createElement(TextField, { label: "Cost (INR)", type: "number", prefix: "\u20B9", value: String(draft.RepairCost || 0), onChange: function (_e, v) { return set('RepairCost', parseFloat(v || '0')); } }),
            React.createElement(DateField, { label: "Next Maintenance Due", value: draft.NextMaintenanceDue, onChange: function (v) { return set('NextMaintenanceDue', v); } }),
        ...
        React.createElement(DialogFooter, null,
            React.createElement(PrimaryButton, { onClick: onConfirm, disabled: saving }, saving ? 'Saving…' : 'Add Entry'),
...
var AssetDetailsForm = function (_a) {
    ...
    var _d = useState(CITY_CODE_FROM_OFFICE[asset ? asset.OfficeCode : defaultOffice] || ''), selectedCity = _d[0], setSelectedCity = _d[1];
    ...
    // Repair dialog
    var _o = useState(false), repairDialogOpen = _o[0], setRepairDialogOpen = _o[1];
    var _p = useState(emptyRepairDraft()), repairDraft = _p[0], setRepairDraft = _p[1];
    var _q = useState(false), savingRepair = _q[0], setSavingRepair = _q[1];
    ...
    useEffect(function () {
        ...
        assetService.getNextSequenceNumber()
            .then(function (seq) { return setPreviewId(AssetIdGenerator.generate(form.Country, form.OfficeCode, form.AssetType, seq)); })
    ...
    var validate = function () {
        ...
        if (form.IsTempAssignment) {
            if (!((_g = form.TempAssignedTo) === null || _g === void 0 ? void 0 : _g.trim()))
                e.TempAssignedTo = 'Temp assignee name is required.';
            if (!((_h = form.TempAssignmentEmail) === null || _h === void 0 ? void 0 : _h.trim()))
                e.TempAssignmentEmail = 'Temp assignee email is required.';
            if (!form.TempStartDate)
                e.TempStartDate = 'Temp start date is required.';
            if (!form.TempEndDate)
                e.TempEndDate = 'Temp end date (return by) is required.';
        ...
        React.createElement("div", { className: styles.form },
            React.createElement(Section, { title: "Identity", icon: React.createElement(TagRegular, null) },
                React.createElement("div", { className: styles.grid },
                    ...
                    React.createElement(Dropdown, { label: "Make *", selectedKey: form.Vendor || '', options: __spreadArray([
                            { key: '', text: 'Select make…' }
                        ], MAKE_OPTIONS.map(function (m) { return ({ key: m, text: m }); }), true), onChange: function (_e, opt) { return set('Vendor', (opt === null || opt === void 0 ? void 0 : opt.key) || ''); }, errorMessage: errors.Vendor }),
                    React.createElement(Dropdown, { label: "Model Type", selectedKey: form.ModelType || '', options: __spreadArray([
                            { key: '', text: 'Select model type…' }
                        ], MODEL_TYPE_OPTIONS.map(function (m) { return ({ key: m, text: m }); }), true), onChange: function (_e, opt) { return set('ModelType', (opt === null || opt === void 0 ? void 0 : opt.key) || ''); } }),
                    React.createElement(Dropdown, { label: "Country *", selectedKey: form.Country || '', disabled: isEdit, options: __spreadArray([
                            ...
                            setSelectedCity('');
                            setForm(function (f) { return (__assign(__assign({}, f), { Country: country, OfficeCode: '' })); });
                        }, errorMessage: errors.Country }),
                    React.createElement(Dropdown, { label: "City *", selectedKey: selectedCity, disabled: isEdit || !form.Country, options: __spreadArray([
                            { key: '', text: form.Country ? 'Select city…' : 'Select country first…' }
                        ], (CITY_OPTIONS[form.Country || ''] || []).map(function (c) { return ({ key: c.key, text: c.text }); }), true), onChange: function (_e, option) {
                            var city = option === null || option === void 0 ? void 0 : option.key;
                            setSelectedCity(city);
                            var offices = OFFICE_OPTIONS_BY_CITY[city] || [];
                            // Auto-select the office when only one exists for this city
                            var autoOffice = offices.length === 1 ? offices[0].key : '';
                            setForm(function (f) { return (__assign(__assign({}, f), { OfficeCode: autoOffice })); });
                        } }),
                    React.createElement(Dropdown, { label: "Site / Office *", selectedKey: form.OfficeCode || '', disabled: isEdit || !selectedCity, options: __spreadArray([
                            { key: '', text: selectedCity ? 'Select site…' : 'Select city first…' }
                        ], (OFFICE_OPTIONS_BY_CITY[selectedCity] || []).map(function (o) { return ({ key: o.key, text: o.text }); }), true), onChange: function (_e, option) { return set('OfficeCode', option === null || option === void 0 ? void 0 : option.key); }, errorMessage: errors.OfficeCode }))),
            React.createElement(Section, { title: "Procurement", icon: React.createElement(MoneyRegular, null) },
                React.createElement("div", { className: styles.grid },
                    ...
                    React.createElement(TextField, { label: "Vendor / Supplier", value: form.ProcurementVendor || '', onChange: function (_e, v) { return set('ProcurementVendor', v || ''); }, maxLength: 200, placeholder: "e.g. Croma, Amazon Business, Ingram Micro\u2026" }),
            ...
            React.createElement(Section, { title: "Warranty & Services", icon: React.createElement(ShieldCheckmarkRegular, null) },
                React.createElement("div", { className: styles.grid },
                    React.createElement(DateField, { label: "Warranty Start Date", value: form.WarrantyStartDate || '', onChange: function (v) { return set('WarrantyStartDate', v); } }),
                    React.createElement(DateField, { label: "Warranty End Date", value: form.WarrantyExpiry || '', onChange: function (v) { return set('WarrantyExpiry', v); }, error: errors.WarrantyExpiry }),
                    React.createElement(DateField, { label: "OEM End of Service Date", value: form.OEMEndOfServiceDate || '', onChange: function (v) { return set('OEMEndOfServiceDate', v); } }),
                    React.createElement(Dropdown, { label: "Type of Warranty", selectedKey: form.WarrantyType || '', options: __spreadArray([
                            { key: '', text: 'Select type…' }
                        ], WARRANTY_TYPE_OPTIONS.map(function (w) { return ({ key: w, text: w }); }), true), onChange: function (_e, opt) { return set('WarrantyType', (opt === null || opt === void 0 ? void 0 : opt.key) || ''); } }),
                    React.createElement(TextField, { label: "Add-on Service", value: form.AddOnService || '', onChange: function (_e, v) { return set('AddOnService', v || ''); }, maxLength: 200, placeholder: "e.g. AppleCare+, HP Care Pack\u2026" }))),
            isEdit && (React.createElement(Section, { title: "Temporary Assignment", icon: React.createElement(ClockRegular, null) },
                React.createElement("div", { style: { marginBottom: 12 } },
                    React.createElement("label", { style: { display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontWeight: 600, fontSize: 14 } },
                        React.createElement("input", { type: "checkbox", checked: !!form.IsTempAssignment, onChange: function (e) {
                                set('IsTempAssignment', e.target.checked);
                                if (!e.target.checked) {
                                    setForm(function (f) { return (__assign(__assign({}, f), { TempAssignedTo: '', TempAssignmentEmail: '', TempAssignmentPurpose: '', TempStartDate: '', TempEndDate: '' })); });
                                }
                            }, style: { width: 16, height: 16, cursor: 'pointer' } }),
                        "This is a temporary assignment"),
                    React.createElement("span", { style: { fontSize: 12, color: '#707070', display: 'block', marginTop: 4, marginLeft: 24 } }, "When enabled, the asset will be tracked as temporarily assigned.")),
                form.IsTempAssignment && (React.createElement("div", { className: styles.grid },
                    React.createElement(TextField, { label: "Temp Assigned To *", value: form.TempAssignedTo || '', onChange: function (_e, v) { return set('TempAssignedTo', v || ''); }, placeholder: "Full name", errorMessage: errors.TempAssignedTo }),
                    React.createElement(TextField, { label: "Temp Assignee Email *", type: "email", value: form.TempAssignmentEmail || '', onChange: function (_e, v) { return set('TempAssignmentEmail', v || ''); }, placeholder: "user@example.com", errorMessage: errors.TempAssignmentEmail }),
                    React.createElement(TextField, { label: "Purpose", value: form.TempAssignmentPurpose || '', onChange: function (_e, v) { return set('TempAssignmentPurpose', v || ''); }, placeholder: "e.g. Trade show, client visit\u2026", maxLength: 500 }),
                    React.createElement(DateField, { label: "Temp Start Date *", value: form.TempStartDate || '', onChange: function (v) { return set('TempStartDate', v); }, error: errors.TempStartDate }),
                    React.createElement(DateField, { label: "Return By (End Date) *", value: form.TempEndDate || '', onChange: function (v) { return set('TempEndDate', v); }, error: errors.TempEndDate }),
                    form.TempReminderSent && (React.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', background: '#dff6dd', borderRadius: 4, fontSize: 13, color: '#107c10' } }, "\u2713 Return reminder has been sent")))))),
            isEdit && (React.createElement(Section, { title: "End of Service", icon: React.createElement(ArchiveRegular, null) },
                React.createElement("div", { className: styles.grid },
                    React.createElement(DateField, { label: "End of Service Date", value: form.EndOfServiceDate || '', onChange: function (v) { return set('EndOfServiceDate', v); } }),
                    React.createElement(Dropdown, { label: "End of Service Reason", selectedKey: form.EndOfServiceReason || '', options: __spreadArray([
                            { key: '', text: 'Select reason…' }
                        ], EOS_REASON_OPTIONS.map(function (r) { return ({ key: r, text: r }); }), true), onChange: function (_e, opt) { return set('EndOfServiceReason', (opt === null || opt === void 0 ? void 0 : opt.key) || ''); } }),
                    React.createElement(TextField, { label: "EOS Remarks", className: styles.fullWidth, multiline: true, rows: 2, value: form.EosRemarks || '', onChange: function (_e, v) { return set('EosRemarks', v || ''); }, maxLength: 1000 })),
                (form.EndOfServiceDate || form.EndOfServiceReason) && (React.createElement("div", { style: { marginTop: 12, padding: '8px 12px', background: '#fff4ce', borderRadius: 4, fontSize: 13, color: '#7d4900' } },
                    "Tip: Consider changing asset status to ",
                    React.createElement("strong", null, "End of Service"),
